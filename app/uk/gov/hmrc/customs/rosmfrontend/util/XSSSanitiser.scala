...
trait XSSSanitiser {
  ...
  def sanitise(input: String): String = {
    @tailrec
    def applySanitisers(filters: Seq[Pattern], sanitizedOuput: String): String = filters match {
      case Nil => sanitizedOuput
      case filter :: tail => applySanitisers(tail, filter.matcher(sanitizedOuput).replaceAll(""))
    }
    applySanitisers(patterns, input)
  }


}

object XSSSanitiser extends XSSSanitiser