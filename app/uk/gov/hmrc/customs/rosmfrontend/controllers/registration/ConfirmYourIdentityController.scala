    ...
    ggAuthorisedUserWithEnrolmentsAction { implicit request => _: LoggedInUserWithEnrolments =>
      {
        ...
        Future.successful(Ok(confirmYourIdentity(form, isInReviewMode = false, journey, Some(organisationType))))
      }
    }
    ...
    ggAuthorisedUserWithEnrolmentsAction { implicit request => loggedInUser: LoggedInUserWithEnrolments =>
      {
        confirmIdentityYesNoAnswer.bindFromRequest.fold(
          formWithErrors =>
            Future.successful(
              BadRequest(
                confirmYourIdentity(formWithErrors, isInReviewMode = false, journey, Some(organisationType))
              )
          ),
          formData =>
            sessionCache.saveHasNino(formData.isYes).map { _ =>
              Redirect(WhatIsYourIdentifierController.form(organisationType, journey))
            }
        )
      }
    }
