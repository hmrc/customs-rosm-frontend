...
import uk.gov.hmrc.customs.rosmfrontend.views.html.subscription.subscription_create_request_not_processed
...
class YouCantUseServiceSpec extends ViewSpec {
  ...
  private val subscriptionCreateView = app.injector.instanceOf[subscription_create_request_not_processed]
  ...
  "You cannot use this service page for users who cannot get an eori" should {
    ...
    "display correct para" in {
      cannotUseService003.body
        .getElementById("para")
        .text mustBe "To apply for an EORI number phone 0300 200 3700 and ask for an assisted digital application form. They're open 8am to 6pm, Monday to Friday (closed bank holidays)."
    }
  }
  ...
  private lazy val cannotUseService003: Document = Jsoup.parse(contentAsString(subscriptionCreateView()))
