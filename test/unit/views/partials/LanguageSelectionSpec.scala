...
import uk.gov.hmrc.customs.rosmfrontend.views.html.partials.language_selection

import java.util.Locale

class language_selectionSpec extends PlaySpec with GuiceOneAppPerSuite {
  ...
  "Language selection template view" should {
    ...
    "give a link to switch to Welsh when current language is English" in {
      val html = language_selection.render(languageMap, langToUrl, None, None, messagesEnglish)
    ...
    "give a link to switch to English when current language is Welsh" in {
      val html = language_selection.render(languageMap, langToUrl, None, None, messagesWelsh)
      contentType(html)     must be("text/html")
    ...
    "show a custom class if it is set" in {
      val html = language_selection.render(languageMap, langToUrl, Some("float--right"), None, messagesWelsh)
      contentType(html)     must be("text/html")
    ...
    "show a data-journey-click attribute for GA if it is set and language is Welsh" in {
      val html = language_selection.render(languageMap, langToUrl, None, Some("appName"), messagesWelsh)
      contentType(html)     must be("text/html")
    ...
    "show a data-journey-click attribute for GA if it is set and language is English" in {
      val html = language_selection.render(languageMap, langToUrl, None, Some("appName"), messagesEnglish)
      contentType(html)     must be("text/html")
    ...
    "show correct current language message when current language is Spanish" in {
      ...
      val html = language_selection.render(mockLanguageMap, langToUrl, None, None, messagesSpanish)
      contentType(html)     must be("text/html")
