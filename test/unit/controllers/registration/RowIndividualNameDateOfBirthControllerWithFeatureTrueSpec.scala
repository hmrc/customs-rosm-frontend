...
class RowIndividualNameDateOfBirthControllerWithFeatureTrueSpec
  ...
  abstract class IndividualNameAndDateOfBirthBehaviour(
    ...
    "middle name" should {
      ...
      passOptionalFieldCheck(middleNameField)(
    ...
    protected def passOptionalFieldCheck(fieldName: String)(
      ...
      "be optional" in testControllerWithModel(formModelGens) { (controllerFixture, individualNameAndDateOfBirth) =>
        ...
        submitForm(formData(individualNameAndDateOfBirth)) { result =>
          status(result) shouldBe SEE_OTHER
          CdsPage(bodyOf(result)).getElementsHtml(webPage.pageLevelErrorSummaryListXPath) shouldBe empty
        }
      }
