...
import uk.gov.hmrc.customs.rosmfrontend.connector.UpdateVerifiedEmailConnector
import uk.gov.hmrc.customs.rosmfrontend.connector.httpparsers._
import uk.gov.hmrc.customs.rosmfrontend.domain.email.{RequestDetail, UpdateVerifiedEmailRequest, UpdateVerifiedEmailResponse}
...
class UpdateVerifiedEmailConnectorSpec
  ...
  private val verifiedEmailResponse = VerifiedEmailResponse(
    UpdateVerifiedEmailResponse(
      ResponseCommon("OK", None, dateTime, Some(List(MessagingServiceParam("name", "value"))))
    )
  )
