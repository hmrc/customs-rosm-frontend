...
object SubscriptionDisplayMessagingService {
  ...
  def validResponseWithNoEmail(typeOfLegalEntity: String, taxPayerID: String = TestData.TaxPayerID): String =
    s"""{
       |  "subscriptionDisplayResponse": {
       |    "responseCommon": {
       |      "status": "OK",
  ...
  def returnSubscriptionDisplayForMigratedTraderInGYEJourney(id: String): Unit =
    stubFor(
      get(urlPathEqualTo("/subscription-display"))
        .withQueryParam("taxPayerID", equalTo(id))
        .withQueryParam("regime", equalTo("CDS"))
        .withQueryParam("acknowledgementReference", matching("[\\w]{32}"))
        .willReturn(
          aResponse()
            .withStatus(OK)
            .withBody(validResponseWithNoEmail(typeOfLegalEntity = "0001"))
            .withHeader(CONTENT_TYPE, JSON)
        )
    )
}
