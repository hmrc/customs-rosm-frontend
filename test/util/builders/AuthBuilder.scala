...
object AuthBuilder {
  ...
  def withAuthorisedUser(
    ...
    val idsRetrievalResult = new ~(new ~(Option(userAffinityGroup), Option(userInternalId)), groupId)
    ...
    when(
      ...
      Future.successful(
        new ~(
          new ~(
            new ~(new ~(new ~(userEmail, userCredentialRole), Option(userAffinityGroup)), Option(userInternalId)),
